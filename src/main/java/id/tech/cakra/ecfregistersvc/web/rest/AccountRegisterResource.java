...
import id.tech.cakra.ecfregistersvc.service.AccountRegisterService;
...
/**
 * REST controller for managing {@link id.tech.cakra.ecfregistersvc.domain.AccountRegister}.
 */
@RestController
@RequestMapping("/api")
public class AccountRegisterResource {
    ...
    private static final String ENTITY_NAME = "ecfregistersvcAccountRegister";
    ...
    public ResponseEntity<AccountRegisterDTO> updateAccountRegister(@Valid @RequestBody AccountRegisterDTO accountRegisterDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, accountRegisterDTO.getId().toString()))
            .body(result);
    }
    ...
    public List<AccountRegisterDTO> getAllAccountRegisters(@RequestParam(required = false) String filter) {
        ...
        if ("accountbankregister-is-null".equals(filter)) {
            log.debug("REST request to get all AccountRegisters where accountBankRegister is null");
            return accountRegisterService.findAllWhereAccountBankRegisterIsNull();
        }
